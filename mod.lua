...
-- ============================ [ ХЕЛПЕР ДЛЯ ЦВЕТА ] ============================
-- В mimgui GetColorU32 не принимает ImVec4 напрямую — используем ColorConvertFloat4ToU32
local function ColU32(v4)
...
local function renderSplashScreen()
    ...
    dl:AddRectFilled(
        imgui.ImVec2(0, 0),
        imgui.ImVec2(resX, resY),
        ColU32(imgui.ImVec4(0, 0, 0, splashAlpha * 0.9))
    )

    local accent = ColU32(imgui.ImVec4(t.Accent.x, t.Accent.y, t.Accent.z, splashAlpha))
    local white  = ColU32(imgui.ImVec4(1, 1, 1, splashAlpha))

    dl:AddText(nil, 40.0, imgui.ImVec2(resX/2 - 250, resY/2 - 40), accent, u8"СТУДИЯ ТОРГОВОЙ АНАЛИТИКИ")
    dl:AddText(nil, 18.0, imgui.ImVec2(resX/2 - 80,  resY/2 + 20), white,  u8"ЗАГРУЗКА ИНТЕРФЕЙСА...")

    if splashStage == 0 then
        splashAlpha = splashAlpha + 0.03
        if splashAlpha >= 1.0 then
            splashAlpha = 1.0
...
local function renderPricesTab()
    ...
    if imgui.BeginChild("PricesChild", imgui.ImVec2(-1, 320), true) then
        ...
        for i, item in ipairs(items) do
            local bg = (i % 2 == 0)
                and t.PriceBg
                or  imgui.ImVec4(t.PriceBg.x + 0.02, t.PriceBg.y + 0.02, t.PriceBg.z + 0.02, 1.0)
...
local function renderSettingsTab()
    ...
    if imgui.BeginChild("SettingsChild", imgui.ImVec2(-1, 320), true) then
        ...
        Icons.DrawCalc(dl, imgui.ImVec2(cur.x + 10, cur.y), 20, ColU32(t.Accent))
        ...
        local res = ((calc_sell[0] - calc_buy[0]) * calc_count[0])
                  - (calc_sell[0] * calc_count[0] * (config.commission / 100))

        imgui.SameLine(350)
        imgui.BeginChild("Res", imgui.ImVec2(230, 80), true)
        imgui.Text(u8"ЧИСТАЯ ПРИБЫЛЬ:")
        imgui.TextColored(
            res >= 0 and imgui.ImVec4(0,1,0,1) or imgui.ImVec4(1,0,0,1),
            string.format("%d РУБ", res)
        )
        imgui.EndChild()
        ...
        imgui.SameLine()
        local btnLabel = isBinding and u8"ЖМИТЕ..." or vkeys.id_to_name(config.menu_key)
        if imgui.Button(btnLabel, imgui.ImVec2(120, 30)) then
            isBinding = true
        end

        imgui.Spacing()
        for i = 0, 2 do
            if imgui.Button(themes[i].name, imgui.ImVec2(145, 35)) then
                currentTheme = i
            end
...
-- ============================ [ МЕНЮ ] ============================
imgui.OnFrame(
    function() return renderMenu[0] or showSplash end,
    function()
        if showSplash then
            renderSplashScreen()
            return
        ...
        if imgui.Begin(u8"TRADE ANALYTICS STUDIO", renderMenu, imgui.WindowFlags.NoCollapse) then
            ...
            Icons.DrawAnalytics(dl, imgui.ImVec2(cp1.x + 20, cp1.y + 15), 20,
                ColU32(currentTab == 0 and t.Accent or t.Text))
            dl:AddText(nil, 18.0, imgui.ImVec2(cp1.x + 55, cp1.y + 15),
                ColU32(t.Text), u8"РЫНОЧНЫЕ ЦЕНЫ")
            ...
            Icons.DrawSettings(dl, imgui.ImVec2(cp2.x + 20, cp2.y + 15), 20,
                ColU32(currentTab == 1 and t.Accent or t.Text))
            dl:AddText(nil, 18.0, imgui.ImVec2(cp2.x + 55, cp2.y + 15),
                ColU32(t.Text), u8"НАСТРОЙКИ СКРИПТА")
            ...
            if imgui.Button("##EXIT", imgui.ImVec2(-1, 45)) then
                renderMenu[0] = false
            end
            Icons.DrawClose(dl, imgui.ImVec2(exitPos.x + 230, exitPos.y + 15), 15, ColU32(t.Text))
            dl:AddText(nil, 18.0, imgui.ImVec2(exitPos.x + 260, exitPos.y + 13), ColU32(t.Text), u8"ЗАКРЫТЬ СТУДИЮ")
...
function main()
    ...
    sampAddChatMessage("{0088FF}[TradeAnalytics] {FFFFFF}Версия 1.5.3 загружена. Язык: {0088FF}Русский{FFFFFF}.", -1)

    while true do
        wait(0)

        -- Перехват клавиши для биндинга
        if isBinding then
            for i = 0, 255 do
